      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 space-y-4">
        ...
        {order.rider && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
            className="rounded-2xl p-6" style={{ background: '#0d0d1a', border: '1px solid rgba(99,102,241,0.25)' }}>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="font-black text-white text-base">{order.rider.name}</p>
                <a href={`tel:${order.rider.phone}`}
                  className="flex items-center gap-1.5 text-sm font-semibold mt-0.5"
                  style={{ color: '#818cf8' }}>
                  <FiPhone size={12} /> {order.rider.phone}
                </a>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="text-xs mb-1" style={{ color: 'rgba(255,255,255,0.35)' }}>{order.rider.bikeName}</p>
